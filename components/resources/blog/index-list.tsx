  ...
  return (
    <section className="bg-dark  py-14 sm:px-8 sm:py-16">
      <div className="mx-auto grid px-6 md:px-10 max-w-7xl gap-5">
        {blogPosts.length === 0 && (
          <div className="rounded-xl border border-light-dark bg-dark p-6 text-muted-foreground">
        ...
        {blogPosts.map((post) => (
          ...
          >
            <div className="flex flex-wrap items-center gap-3 tracking-tight text-muted-foreground">
              <span>{post.readTime}</span>
            </div>
            <h2 className="mt-5 text-2xl font-semibold tracking-tight text-heading transition-colors group-hover:text-brand sm:text-2xl">
              {post.title}
            </h2>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 text-sm">
              ...
              <span className="text-muted-foreground">Updated {post.updated}</span>
            </div>
          </Link>
        ))}
      </div>
